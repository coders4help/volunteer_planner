...
def get_memberships_by_role(membership_queryset):
    ...
    qs = membership_queryset.order_by("membership__role").values_list(
        "membership__role", "pk"
    )
...
def filter_queryset_by_membership(
    qs,
    user,
    ...
    if qs.model == models.Organization:
        ...
        qs = qs.filter(Q(pk__in=user_facilities) | Q(organization_id__in=user_orgs))
    else:
            ...
            qs = qs.filter(
                ...
                | Q(
                    **{
                        "shift__"
                        + facility_filter_fk
                        + "__organization_id__in": user_orgs
                    }
                )
            )
        else:
...
class MembershipFilteredAdmin(admin.ModelAdmin):
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        if request.user.is_superuser:
            ...
            if not ("facility" in readonly and "organization" in readonly):
                ...
                if (
                    len(user_facilities) <= 1
                    and hasattr(obj, "facility")
    ...
    def get_list_display_links(self, request, list_display):
        list_display_links = list(
            super(MembershipFilteredAdmin, self).get_list_display_links(
    ...
    get_edit_link.short_description = _("edit")
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        if "facility" in form.base_fields:
            ...
            user_facilities = filter_queryset_by_membership(facilities, request.user)
    ...
    def get_queryset(self, request):
        ...
        return filter_queryset_by_membership(
            qs,
            user=request.user,
            facility_filter_fk=fac_filter,
            organization_filter_fk=org_filter,
        )
    ...
    def get_field_queryset(self, db, db_field, request):
        ...
        if db_field.remote_field.model in (
            models.Facility,
            models.Organization,
            models.Task,
            models.Workplace,
...
class OrganizationAdmin(MembershipFilteredAdmin):
    ...
    get_contact_info.short_description = _("contact info")
...
class FacilityAdmin(MembershipFilteredAdmin):
    ...
    list_filter = (("organization", MembershipFieldListFilter),)
    raw_id_fields = ("members",)
    search_fields = ("name",)
    widgets = {
        "description": CKEditorWidget(),
...
class WorkplaceAdmin(MembershipFilteredAdmin):
    ...
    list_display = ("name", "facility", "priority", "get_description")
    list_filter = (("facility", MembershipFieldListFilter),)
    search_fields = ("name",)
