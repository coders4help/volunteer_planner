...
def find_cta(context, call_to_action):
    ...
    assert call_to_action in element.text, (
        'Call to action "%s" was not found' % call_to_action
    )
...
def find_section(context, heading_to_look_for):
    headings = [
        h.text
        for h in context.browser.find_elements_by_xpath("//h1 | //h2 | //h3")
        if h.text
    ]
...
def find_button(context, btn_label):
    btn_labels = [
        btn.text
        for btn in context.browser.find_elements_by_class_name("btn")
        if btn.text
    ]
    assert btn_label in btn_labels, (
        'Cannot find a button with the label "%s"' % btn_label
    )
...
def find_nav_bar(context):
    navigation_link_labels = [
        link.text
        for link in context.browser.find_elements_by_xpath(
            '//div[@id="footer-nav"]//li//a'
        )
        if link.text
    ]
