...
def assert_shift_conflict_count(
    shift,
    hard_conflict_count,
    soft_conflict_count,
...
class ShiftTestCase(TestCase):
    ...
    def setUp(self):
        ...
        ShiftHelper.objects.create(
            user_account=self.user_account, shift=self.morning_shift
        )

        self.evening_shift = create_shift(18, 21)
        ShiftHelper.objects.create(
            user_account=self.user_account, shift=self.evening_shift
        )

        self.short_shift = create_shift(1, 2)
        ShiftHelper.objects.create(
            user_account=self.user_account, shift=self.short_shift
        )
...
class FacilityTestCase(TestCase):
    def test_shift_manager_for_facility(self):
        ...
        tomorrow_shift = ShiftFactory.create(
            facility=facility,
            task=task,
            workplace=workplace,
        ...
        assert (
            shifts.count() == 1
        ), "only 1 shift should be found with Shifts.open_shifts"
        shift = shifts.get()
