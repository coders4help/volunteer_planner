...
class GenerateExcelSheet:
    ...
    def send_file(self):
        mail = EmailMessage()
        mail.body = "Hallo " + self.mailer.first_name +" "+self.mailer.last_name+" Anbei die Liste zum Dienstplan der Freiwilligen. Dies ist ein Service von volunteer-planner.org"
        mail.subject = "Dienstplan fuer den "+self.needs[0].time_period_from.date_time.strftime("%d.%m.%Y")+" der Freiwilligen in der Unterkunft "+self.needs[0].location.name
